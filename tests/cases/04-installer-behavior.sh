...
_test_binary_present_noop() {
  ...
  local sbx
  sbx=$(new_sandbox "noop-$plugin")
  local log="$sbx/install.log"
  : >"$log"
  printf '#!/usr/bin/env bash\nexit 0\n' >"$sbx/bin/$binary"
  chmod +x "$sbx/bin/$binary"
  if ! _run_installer "$sbx" "$plugin" >"$sbx/out.log" 2>&1; then
    echo "exit non-zero with binary present:"
    cat "$sbx/out.log"
    return 1
  fi
  if [[ -s "$log" ]]; then
    echo "expected zero install calls, got:"
    cat "$log"
    return 1
  fi
}
...
_test_primary_install_brew() {
  ...
  calls=$(wc -l <"$log")
  if ((calls != 1)); then
    echo "expected exactly 1 install call, got $calls:"
    cat "$log"
    return 1
  fi
  local line
  ...
  if [[ "$line" != "brew install $formula" ]]; then
    echo "expected 'brew install $formula', got '$line'"
    return 1
  fi
}
...
_test_primary_install_npm() {
  ...
  if [[ "$line" != "npm install -g $pkg" ]]; then
    echo "expected 'npm install -g $pkg', got '$line'"
    return 1
  fi
}
...
_test_cue_binary_install() {
  ...
  if ! _run_installer "$sbx" cue-lsp >"$sbx/out.log" 2>&1; then
    echo "cue binary install exited non-zero:"
    cat "$sbx/out.log"
    return 1
  fi
  if ! grep -q '^curl ' "$log"; then
    echo "no curl call recorded"
    cat "$log"
    return 1
  fi
  if ! grep -q '^tar ' "$log"; then
    echo "no tar call recorded"
    cat "$log"
    return 1
  fi
  ...
  if ! grep -F 'releases/download/v0.16.0/' "$log" >/dev/null; then
    echo "cue download URL missing pinned version v0.16.0:"
    cat "$log"
    return 1
  fi
  # OS/arch must come from the mocked uname, not the host.
  if ! grep -F 'cue_v0.16.0_linux_amd64.tar.gz' "$log" >/dev/null; then
    echo "cue URL did not reflect mocked uname (linux/x86_64 -> amd64):"
    cat "$log"
    return 1
  fi
}
...
_test_regal_binary_install() {
  local sbx
  sbx=$(new_sandbox "regal-binary")
  local log="$sbx/install.log"
  : >"$log"
  make_uname_mock "$sbx/bin" Linux x86_64
  make_curl_mock "$sbx/bin" "$log"
  if ! _run_installer "$sbx" regal-lsp >"$sbx/out.log" 2>&1; then
    echo "regal binary install exited non-zero:"
    cat "$sbx/out.log"
    return 1
  fi
  local curl_calls
  ...
  if ((curl_calls != 1)); then
    echo "expected exactly 1 curl call, got $curl_calls:"
    cat "$log"
    return 1
  fi
  # grep -F so $sbx isn't interpreted as a regex.
  if ! grep -F -- "-o $sbx/home/.local/bin/regal" "$log" >/dev/null; then
    echo "curl call did not target the expected install path:"
    cat "$log"
    return 1
  fi
  ...
  if ! grep -F 'releases/download/v0.39.0/' "$log" >/dev/null; then
    echo "regal download URL missing pinned version v0.39.0:"
    cat "$log"
    return 1
  fi
  if ! grep -F 'regal_Linux_x86_64' "$log" >/dev/null; then
    echo "regal URL did not reflect mocked uname (Linux/x86_64):"
    cat "$log"
    return 1
  fi
}
...
_test_binary_download_failure() {
  local plugin="$1"
  local sbx
  sbx=$(new_sandbox "binary-fail-$plugin")
  local log="$sbx/install.log"
  : >"$log"
  make_uname_mock "$sbx/bin" Linux x86_64
  ...
  if ((rc == 0)); then
    echo "expected non-zero exit on curl failure; got 0"
    cat "$sbx/out.log"
    return 1
  fi
  if ! grep -F 'Binary download failed' "$sbx/out.log" >/dev/null; then
    echo "expected 'Binary download failed' message; got:"
    cat "$sbx/out.log"
    return 1
  fi
}
...
_test_npm_failure_propagates() {
  local plugin="$1"
  local sbx
  sbx=$(new_sandbox "npm-fail-$plugin")
  local log="$sbx/install.log"
  : >"$log"
  ...
  _run_installer "$sbx" "$plugin" >"$sbx/out.log" 2>&1 || rc=$?
  if ((rc == 0)); then
    echo "expected non-zero exit on npm failure; got 0"
    cat "$sbx/out.log"
    return 1
  fi
  if ! grep -F 'npm install failed' "$sbx/out.log" >/dev/null; then
    echo "expected 'npm install failed' message; got:"
    cat "$sbx/out.log"
    return 1
  fi
}
...
tc_install_darwin_regal() {
  ...
  if grep -q '^curl ' "$log"; then
    echo "Darwin path unexpectedly invoked curl:"
    cat "$log"
    return 1
  fi
  local line
  ...
  if [[ "$line" != "brew install styrainc/tap/regal" ]]; then
    echo "expected 'brew install styrainc/tap/regal', got '$line'"
    return 1
  fi
}
...
tc_install_mkdir_lock_fallback() {
  ...
  calls=$(wc -l <"$log")
  if ((calls != 1)); then
    echo "expected 1 install call via mkdir-lock branch, got $calls:"
    cat "$log"
    return 1
  fi
...
tc_install_failure_propagates() {
  ...
  if ! grep -F 'brew install failed' "$sbx/out.log" >/dev/null; then
    echo "expected 'brew install failed' message; got:"
    cat "$sbx/out.log"
    return 1
  fi
}
...
tc_install_post_check_missing_binary() {
  ...
  if ! grep -F 'Not in PATH after install' "$sbx/out.log" >/dev/null; then
    echo "expected post-install message; got:"
    cat "$sbx/out.log"
    return 1
  fi
}

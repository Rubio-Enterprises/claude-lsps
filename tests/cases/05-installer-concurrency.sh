...
_test_concurrency_brew() {
  ...
  local timing="$sbx/timing.tsv"; : > "$timing"
  ...
  if (( installing_lines != 1 )); then
    ...
    echo "--- out1 ---"; cat "$sbx/out1.log"
    echo "--- out2 ---"; cat "$sbx/out2.log"
    return 1
  fi
}
...
_test_concurrency_brew_mkdir() {
  local plugin="$1" target="$2"
  local sbx; sbx=$(new_sandbox "conc-brew-mkdir-$plugin")
  local log="$sbx/install.log"; : > "$log"
  local timing="$sbx/timing.tsv"; : > "$timing"
  ...
  if compgen -G "$sbx/lock-*.lock.d" >/dev/null; then
    echo "mkdir lock dir was not cleaned up:"; compgen -G "$sbx/lock-*.lock.d"
    return 1
  fi
}
